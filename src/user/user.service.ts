...
export class UserService {
  constructor(
    ...
    @InjectRepository(ParentEntity) private readonly parentRepository: Repository < ParentEntity > ,
    @InjectRepository(SitterEntity) private readonly sitterRepository: Repository < SitterEntity >
  ) {}
  ...
  async createParent(userData: ParentDto) {
    ...
    try {
      let user = await this.createUser(result);
      ...
      await this.userRepository.update({primaryId:user.primaryId},{parent : parent});
    } catch (error) {
  ...
  async addParent(req, userData: ParentAdd) {
    ...
    const isExist = await this.parentRepository.findOne({where :{ primaryId : user.primaryId }});
    if(isExist) {
  ...
  async addSitter(req, userData: SitterAdd) {
    ...
    const isExist = await this.sitterRepository.findOne({where :{ primaryId : user.primaryId }});
    if(isExist) {
      throw new NotAcceptableException();
    }
    let sitter = new SitterEntity();
  ...
  async changeInfo(req, userData : ChangeDto):Promise<Info> {
    ...
    try {
      ...
      if((userData.childAge || userData.application) && user.parent){
        ...
        await this.parentRepository.update({primaryId:user.primaryId},{ ...updateParent});
      }
      if((userData.careableAge || userData.introduction) && user.sitter){
        ...
        await this.sitterRepository.update({primaryId:user.primaryId},{ ...updateSitter});
      }
  ...
  async findOne(userId:string) : Promise<UserEntity | undefined> {
    const findUser = await this.userRepository.findOne({where : { userId },relations:['parent','sitter']});
    return findUser;
  }
}